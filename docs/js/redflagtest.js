document.addEventListener('DOMContentLoaded', function () { 
    ...
    const globalBack = document.getElementById('globalBack'); // Global back button

    // Replace the previous globalBack event listener with this:
...
function showCustomModal() {
    ...
    // Create the modal container (small tab)
    ...
    // Modal message
    modal.innerHTML = `<p>Voulez-vous vraiment quitter le test ?<br>Votre progression sera perdue.</p>`;
    ...
    // Append buttons to their container, then the container to the modal
    btnContainer.appendChild(quitButton);
    btnContainer.appendChild(cancelButton);
    modal.appendChild(btnContainer);

    // Append the modal to the overlay and add it to the document
    ...
    function displayQuestions(questions) {
        ...
        function showResult() {
            ...
            // Display the results along with a Skip button (which acts as the restart button)
            quizContainer.innerHTML = `
                <div style="text-align: center; padding: 20px;">
                    ...
                    <button id="skipRestart" style="margin-top: 20px; padding: 10px 20px;">Recommencer le test</button>
                </div>
            `;

            // Restart the test when the Skip button on the results page is clicked.
            document.getElementById("skipRestart").addEventListener("click", function() {
                document.querySelector('.form-container').style.display = 'block';
                quizSection.style.display = 'none';
                form.reset();
            });
